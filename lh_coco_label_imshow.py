...
#         bbox = []
#         cid = 0
#         img_name = img['file_name'].replace('jpg','png')
#         img_id = img['id']
#         flag=0
    ...
    for i in range(len(label)):
        ...
        if label[i]['boxes'] == [] or ('Fibrosis' not in label[i]['syms'] ):
            ...
            for j in range(len(label[i]["syms"])):
                ...
                stat[bbox_name]['loc'][0] = min(stat[bbox_name]['loc'][0], y1/1024)
                stat[bbox_name]['loc'][1] = max(stat[bbox_name]['loc'][1], y2/1024)
                stat[bbox_name]['loc'][2] = min(stat[bbox_name]['loc'][2], x1/1024)
                stat[bbox_name]['loc'][3] = max(stat[bbox_name]['loc'][3], x2/1024)
